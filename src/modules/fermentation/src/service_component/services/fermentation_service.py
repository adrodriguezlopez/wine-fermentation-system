...
class FermentationService(IFermentationService):
    ...
    async def create_fermentation_with_blend(
        ...
        validation_result = self._validator.validate_creation_data(
        ...
        with LogTimer(logger, "create_fermentation_with_blend"):
            ...
            async with uow:
                ...
                fermentation = await uow.fermentation_repo.create(
                    winery_id=winery_id, data=data.fermentation_data
                )
    ...
    async def get_fermentations_by_winery(
        ...
        if data_source:
            fermentations = await self._fermentation_repo.list_by_data_source(
                winery_id=winery_id, data_source=data_source, include_deleted=False
            )
            ...
            fermentations = await self._fermentation_repo.get_by_winery(
                winery_id=winery_id, include_completed=include_completed
            )
    ...
    async def update_status(
        self, fermentation_id: int, winery_id: int, new_status: str, user_id: int
    ) -> bool:
        """
        ...
        with LogTimer(logger, "update_status_service"):
            ...
            validation_result = self._validator.validate_status_transition(
                current_status=fermentation.status, new_status=new_status
            )
            ...
            if not validation_result.is_valid:
                ...
                raise ServiceValidationError(
                    "; ".join(error_messages), errors=validation_result.errors
                )
            ...
            if updated_fermentation is None:
                raise NotFoundError(
                    f"Fermentation {fermentation_id} not found after update"
                )
    ...
    def validate_creation_data(self, data: FermentationCreate):
        """
