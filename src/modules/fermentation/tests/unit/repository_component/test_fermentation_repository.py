...
# Import domain entities from their canonical locations
...
class TestFermentationRepository:
    ...
    async def test_get_by_id_returns_fermentation_when_found(self, repository, mock_session_manager):
        ...
        # Verify result
        assert isinstance(result, Fermentation)
        assert result.id == 1
        assert result.winery_id == 1
        assert result.status == FermentationStatus.ACTIVE

    @pytest.mark.asyncio
    async def test_update_status_returns_false_when_not_found(self, repository, mock_session_manager):
        """Test that update_status returns False when fermentation doesn't exist."""
        manager, mock_session = mock_session_manager
    ...
    async def test_update_status_returns_true_when_successful(self, repository, mock_session_manager):
        """Test that update_status returns True when update succeeds."""
        manager, mock_session = mock_session_manager
        ...
        assert mock_sql_fermentation.status == FermentationStatus.COMPLETED.value
    ...
    async def test_get_by_status_returns_list_of_fermentations(self, repository, mock_session_manager):
        ...
        mock_fermentation1.fermented_by_user_id = 1
        mock_fermentation1.status = "ACTIVE"
        mock_fermentation1.vintage_year = 2024
        mock_fermentation1.yeast_strain = "Yeast1"
        mock_fermentation1.input_mass_kg = 100.0
        mock_fermentation1.initial_sugar_brix = 20.0
        mock_fermentation1.initial_density = 1.08
        mock_fermentation1.start_date = datetime.utcnow()
        mock_fermentation1.vessel_code = "V1"
        mock_fermentation1.created_at = datetime.utcnow()
        mock_fermentation1.updated_at = datetime.utcnow()

        mock_fermentation2 = Mock()
        mock_fermentation2.id = 2
        mock_fermentation2.winery_id = 1
        mock_fermentation2.fermented_by_user_id = 1
        mock_fermentation2.status = "ACTIVE"
        mock_fermentation2.vintage_year = 2024
        mock_fermentation2.yeast_strain = "Yeast2"
        mock_fermentation2.input_mass_kg = 150.0
        mock_fermentation2.initial_sugar_brix = 22.0
        mock_fermentation2.initial_density = 1.09
        mock_fermentation2.start_date = datetime.utcnow()
        mock_fermentation2.vessel_code = "V2"
        mock_fermentation2.created_at = datetime.utcnow()
        mock_fermentation2.updated_at = datetime.utcnow()
        ...
        assert len(result) == 2
        assert all(isinstance(f, Fermentation) for f in result)
    ...
    async def test_get_by_winery_returns_all_fermentations_for_winery(self, repository, mock_session_manager):
        ...
        mock_fermentation.fermented_by_user_id = 1
        mock_fermentation.status = "ACTIVE"
        mock_fermentation.vintage_year = 2024
        mock_fermentation.yeast_strain = "Test Yeast"
        mock_fermentation.input_mass_kg = 100.0
        mock_fermentation.initial_sugar_brix = 20.0
        mock_fermentation.initial_density = 1.08
        mock_fermentation.start_date = datetime.utcnow()
        mock_fermentation.vessel_code = "V1"
        mock_fermentation.created_at = datetime.utcnow()
        mock_fermentation.updated_at = datetime.utcnow()
        ...
        assert isinstance(result, list)
        assert len(result) == 1
        assert isinstance(result[0], Fermentation)
