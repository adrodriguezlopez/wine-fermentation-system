...
class BusinessRuleValidationService(IBusinessRuleValidationService):
    ...
    async def validate_sugar_trend(
        ...
        previous_sample = await self.sample_repository.get_latest_sample_by_type(
            fermentation_id, SampleType.SUGAR
        )
    ...
    async def validate_temperature_range(
        self, temperature: float, fermentation_id: int
    ) -> ValidationResult:
        """
        ...
        fermentation_range = (
            await self.fermentation_repository.get_fermentation_temperature_range(
                fermentation_id
            )
        )
