...
from src.shared.infra.database.fastapi_session import (
    initialize_database,
    close_database,
)
...
def _get_db_url() -> str:
    ...
    url = os.getenv(
        "DATABASE_URL",
        "postgresql+asyncpg://postgres:postgres@localhost:5433/wine_fermentation",
    )
...
def create_app() -> FastAPI:
    ...
    app.include_router(
        fermentation_router, prefix=API_V1_PREFIX, tags=["fermentations"]
    )

    # Protocol routers (Phase 2 API)
    app.include_router(protocol_router, prefix=API_V1_PREFIX, tags=["protocols"])
    app.include_router(
        protocol_step_router, prefix=API_V1_PREFIX, tags=["protocol-steps"]
    )
    app.include_router(
        protocol_execution_router, prefix=API_V1_PREFIX, tags=["protocol-executions"]
    )
    app.include_router(
        step_completion_router, prefix=API_V1_PREFIX, tags=["step-completions"]
    )
    ...
    app.include_router(
        historical_router, prefix=API_V1_PREFIX
    )  # ADR-032: /api/v1/fermentation/historical
    ...
    async def health_check():
        ...
        return {"status": "healthy", "service": "fermentation", "version": "1.0.0"}
