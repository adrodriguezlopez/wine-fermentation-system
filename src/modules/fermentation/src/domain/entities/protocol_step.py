...
class ProtocolStep(BaseEntity):
    ...
    __table_args__ = (
        UniqueConstraint(
            "protocol_id", "step_order", name="uq_protocol_steps__protocol_order"
        ),
    ...
    # Timing
    expected_day: Mapped[int] = mapped_column(
        Integer, nullable=False
    )  # 0 = crush day, 1 = next day, etc.
    tolerance_hours: Mapped[int] = mapped_column(
    ...
    criticality_score: Mapped[float] = mapped_column(
        Float, default=1.0, nullable=False
    )  # 0-100

    # Flexibility
    can_repeat_daily: Mapped[bool] = mapped_column(
