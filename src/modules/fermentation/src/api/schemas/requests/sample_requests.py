...
class SampleCreateRequest(BaseModel):
    ...
    recorded_at: datetime = Field(..., description="Timestamp when sample was recorded")
...
class SampleUpdateRequest(BaseModel):
    ...
    sample_type: Optional[str] = Field(
        None, min_length=1, max_length=50, description="Type of sample measurement"
    )

    value: Optional[float] = Field(None, description="Measured value")

    units: Optional[str] = Field(
        None, min_length=1, max_length=20, description="Units of measurement"
    )

    recorded_at: Optional[datetime] = Field(
        None, description="Timestamp when sample was recorded"
    )
