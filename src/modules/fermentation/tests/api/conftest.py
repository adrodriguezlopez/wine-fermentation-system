...
def create_test_app(
    user_override: UserContext = None, db_override: AsyncSession = None
):
    """
    ...
    from src.shared.infra.database.fastapi_session import (
        get_db_session as real_get_db_session,
    )
    ...
    app.include_router(
        samples_router, prefix=API_V1_PREFIX
    )  # New: non-nested sample endpoints
...
def mock_user_context():
    ...
    """
    return UserContext(
        user_id=1, winery_id=1, email="test@winery.com", role=UserRole.WINEMAKER
    )
...
async def client(mock_user_context, override_db_session):
    ...
    """
    app = create_test_app(
        user_override=mock_user_context, db_override=override_db_session
    )
...
async def unauthenticated_client(override_db_session):
    ...
    """
    app = create_test_app(
        user_override=None, db_override=override_db_session
    )  # No auth override
    return TestClient(app)
