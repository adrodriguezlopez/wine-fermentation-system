...
class StepCompletion(BaseEntity):
    ...
    execution_id: Mapped[int] = mapped_column(
        ForeignKey("protocol_executions.id"), nullable=False, index=True
    )
    step_id: Mapped[int] = mapped_column(
        ForeignKey("protocol_steps.id"), nullable=False, index=True
    )
    completed_by_user_id: Mapped[Optional[int]] = mapped_column(
        Integer, nullable=True
    )  # Audit: who completed the step - no FK to avoid module dependencies
    verified_by_user_id: Mapped[Optional[int]] = mapped_column(
        Integer, nullable=True
    )  # Audit: who verified - no FK to avoid module dependencies
    ...
    # Schedule compliance
    is_on_schedule: Mapped[Optional[bool]] = mapped_column(
        Boolean, nullable=True
    )  # Within tolerance?
    days_late: Mapped[int] = mapped_column(
        Integer, default=0, nullable=False
    )  # 0 if on-time, >0 if late
    ...
    skip_notes: Mapped[Optional[str]] = mapped_column(
        Text, nullable=True
