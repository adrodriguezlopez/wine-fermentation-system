...
class FermentationResponse(BaseModel):
    ...
    vessel_code: Optional[str] = Field(
        None, description="Fermentation vessel code", max_length=50
    )

    # Initial measurements
    input_mass_kg: float = Field(..., description="Initial grape mass in kg", gt=0)
    initial_sugar_brix: float = Field(
        ..., description="Initial sugar content in Brix", ge=0, le=50
    )
...
class TimelineResponse(BaseModel):
    ...
    first_sample_date: Optional[datetime] = Field(
        None, description="Date of first sample"
    )
    last_sample_date: Optional[datetime] = Field(
        None, description="Date of most recent sample"
    )
...
class StatisticsResponse(BaseModel):
    ...
    duration_days: Optional[float] = Field(
        None, description="Duration in days (from start to last sample)"
    )

    # Sample metrics
    total_samples: int = Field(
        ..., description="Total number of samples recorded", ge=0
    )
    ...
    initial_sugar: Optional[float] = Field(
        None, description="Initial sugar value (°Brix)"
    )
    latest_sugar: Optional[float] = Field(
        None, description="Most recent sugar value (°Brix)"
    )
    ...
    # Sample frequency
    avg_samples_per_day: Optional[float] = Field(
        None, description="Average samples recorded per day"
    )
