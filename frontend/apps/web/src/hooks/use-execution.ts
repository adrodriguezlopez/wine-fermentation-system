...
export function useExecution(executionId: number | undefined, fermentationStatus?: string) {
  return useQuery({
    ...
    queryFn: () => apiClient.fermentation.get(`/api/v1/executions/${executionId}`).then(r => r.data),
    enabled: executionId !== undefined,
    refetchInterval: fermentationStatus === 'COMPLETED' ? false : 30_000,
  })
}
...
export function useExecutionAlerts(executionId: number | undefined, executionStatus?: string) {
  return useQuery({
    ...
    queryFn: () => apiClient.fermentation.get(`/api/v1/executions/${executionId}/alerts`).then(r => r.data),
    enabled: executionId !== undefined,
    refetchInterval: executionStatus === 'COMPLETED' ? false : 30_000,
  })
}
...
export function useExecutionCompletions(executionId: number | undefined) {
  return useQuery({
    ...
    queryFn: () => apiClient.fermentation.get(`/api/v1/executions/${executionId}/completions`).then(r => r.data),
    enabled: executionId !== undefined,
  })
}
...
export function useAcknowledgeAlert() {
  ...
  return useMutation({
    mutationFn: (alertId: number) =>
      apiClient.fermentation.post(`/api/v1/alerts/${alertId}/acknowledge`).then(r => r.data),
...
export function useDismissAlert() {
  ...
  return useMutation({
    mutationFn: (alertId: number) =>
      apiClient.fermentation.post(`/api/v1/alerts/${alertId}/dismiss`).then(r => r.data),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['execution'] }),
  })
}
