...
from service_component.interfaces import IValidationOrchestrator


def test_validation_orchestrator_interface_methods():
    """Test that IValidationOrchestrator has all expected methods."""

    # Expected methods in our current interface
    expected_methods = {"validate_sample_complete", "validate_sample_batch"}

    # Get actual methods from interface
    actual_methods = set()
    for name, method in inspect.getmembers(
        IValidationOrchestrator, predicate=inspect.isfunction
    ):
