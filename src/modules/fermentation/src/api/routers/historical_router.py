...
router = APIRouter(prefix="/fermentation/historical", tags=["Historical Data"])
    ...
    try:
        ...
        if start_date_from:
            fermentations = [
                ...
                and (
                    f.start_date.date()
                    if isinstance(f.start_date, datetime)
                    else f.start_date
                )
                >= start_date_from
            ]
        ...
        if fruit_origin_id:
            fermentations = [
                f for f in fermentations if f.fruit_origin_id == fruit_origin_id
            ]
...
async def extract_patterns(
    ...
    start_date: Optional[date] = Query(
        None, description="Start date for analysis range"
    ),
...
async def get_dashboard_statistics(
    ...
    """
    logger.info("Getting dashboard statistics", extra={"winery_id": winery_id})
    ...
    try:
        ...
        logger.info(
            ...
            extra={
                "winery_id": winery_id,
                "total_fermentations": stats.total_fermentations,
            },
        )
...
async def trigger_import(
    ...
    added in future phases (ADR-031 ETL Pipeline Integration).
    """
    logger.info("Triggering ETL import job", extra={"winery_id": winery_id})
...
async def get_import_job_status(
    job_id: int, winery_id: int = Depends(get_winery_id)
) -> ImportJobResponse:
    ...
    """
    logger.info(
        "Getting import job status", extra={"winery_id": winery_id, "job_id": job_id}
    )
