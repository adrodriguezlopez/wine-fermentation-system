...
class TestGetHistoricalFermentations:
    ...
    def service(self, mock_fermentation_repo: Mock, mock_sample_repo: Mock):
        ...
        from src.modules.fermentation.src.service_component.services.historical.historical_data_service import (
            HistoricalDataService,
        )

        return HistoricalDataService(
            fermentation_repo=mock_fermentation_repo, sample_repo=mock_sample_repo
        )

    @pytest.mark.asyncio
    async def test_get_historical_fermentations_returns_list_with_filters(
        self, service, mock_fermentation_repo: Mock
    ):
        ...
        mock_fermentations = [
            Mock(
                ...
                start_date=filters["start_date_from"],
                fruit_origin_id=filters["fruit_origin_id"],
            ...
            Mock(
                ...
                winery_id=winery_id,
                data_source="HISTORICAL",
                start_date=filters["start_date_to"],
                fruit_origin_id=filters["fruit_origin_id"],
                status=filters["status"],
            ),
        ]
        ...
        result = await service.get_historical_fermentations(
            winery_id=winery_id, filters=filters, limit=limit, offset=offset
        )
        ...
        mock_fermentation_repo.list_by_data_source.assert_called_once_with(
            winery_id=winery_id, data_source="HISTORICAL", include_deleted=False
        )
    ...
    async def test_get_historical_fermentations_with_minimal_filters(
        ...
        # Act
        result = await service.get_historical_fermentations(
            winery_id=winery_id, filters={}, limit=100, offset=0
        )
    ...
    async def test_get_historical_fermentations_applies_pagination(
        ...
        all_fermentations = [
            Mock(spec=Fermentation, id=i, winery_id=winery_id, data_source="HISTORICAL")
        ...
        # Act
        result = await service.get_historical_fermentations(
            winery_id=winery_id, filters={}, limit=limit, offset=offset
        )
...
class TestGetFermentationSamples:
    ...
    async def test_get_samples_returns_list_when_fermentation_found(
        self, service, mock_fermentation_repo: Mock, mock_sample_repo: Mock
    ):
...
class TestExtractPatterns:
    ...
    async def test_extract_patterns_returns_empty_when_no_data(
        ...
        # Act
        result = await service.extract_patterns(
            winery_id=winery_id, fruit_origin_id=None, date_range=None
        )

        # Assert
        assert result["total_fermentations"] == 0
        assert (
            result["avg_initial_density"] is None or result["avg_initial_density"] == 0
        )
