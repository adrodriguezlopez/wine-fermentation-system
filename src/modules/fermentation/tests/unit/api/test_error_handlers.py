...
from src.modules.fermentation.src.api.error_handlers import (
...
async def test_handle_service_errors_reraises_domain_error():
    ...
    async def domain_error_func():
        raise FermentationNotFound(
            message="Fermentation not found", fermentation_id=123
        )
...
async def test_register_fermentation_already_completed_handler():
    ...
    exc = FermentationAlreadyCompleted(
        message="Fermentation already completed", fermentation_id=123
    )
...
async def test_register_invalid_sample_value_handler():
    ...
    exc = InvalidSampleValue(
        message="Brix cannot be negative", sample_id=789, field="brix", value=-5.0
    )
...
async def test_register_all_handlers_at_once():
    ...
    for exc_class in expected_handlers:
        assert (
            exc_class in registered_handlers
        ), f"{exc_class.__name__} handler not registered"
