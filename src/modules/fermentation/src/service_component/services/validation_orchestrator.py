    ...
    async def validate_sample_complete(
        ...
        logger.debug(
            ...
            sample_type=(
                ...
                if isinstance(new_sample.sample_type, str)
                else new_sample.sample_type.value
            ),
        )
        ...
        chronology_result = await self.chronology_validator.validate_sample_chronology(
            fermentation_id=fermentation_id, new_sample=new_sample
        )
        ...
        value_result = self.value_validator.validate_sample_value(
            sample_type=new_sample.sample_type, value=new_sample.value
        )
        ...
        if (
            ...
            or new_sample.sample_type == SampleType.SUGAR
        ):
            if new_sample.value is not None:
                business_rules_result = (
                    await self.business_rules_validator.validate_sugar_trend(
            ...
            or new_sample.sample_type == SampleType.TEMPERATURE
        ):
