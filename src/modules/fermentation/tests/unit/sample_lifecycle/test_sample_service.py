...
from src.modules.fermentation.src.service_component.services.sample_service import (
    SampleService,
)
...
@pytest.fixture
def sample_service(
    mock_sample_repo, mock_validation_orchestrator, mock_fermentation_repo
):
...
class TestAddSample:
    ...
    async def test_add_sample_happy_path(
        ...
        mock_validation_orchestrator.validate_sample_complete.return_value = (
            valid_validation_result
        )
        ...
        result = await sample_service.add_sample(
            fermentation_id=1, winery_id=100, user_id=1, data=sample_create_dto
        )

        # Assert
        assert result == sample_entity
        mock_fermentation_repo.get_by_id.assert_awaited_once_with(
            fermentation_id=1, winery_id=100
        )
        sample_service._create_sample_entity.assert_called_once_with(
            fermentation_id=1, user_id=1, data=sample_create_dto
        )
    ...
    async def test_add_sample_fermentation_not_found(
        self, sample_service, mock_fermentation_repo, sample_create_dto
    ):
        ...
        with pytest.raises(NotFoundError, match="Fermentation .* not found"):
            await sample_service.add_sample(
                fermentation_id=999, winery_id=100, user_id=1, data=sample_create_dto
            )
...
class TestGetSample:
    ...
    async def test_get_sample_happy_path(
        self, sample_service, mock_sample_repo, sample_entity
    ):
        ...
        result = await sample_service.get_sample(
            sample_id=1, fermentation_id=1, winery_id=100
        )
    ...
    @pytest.mark.asyncio
    async def test_get_sample_not_found(self, sample_service, mock_sample_repo):
        ...
        mock_sample_repo.get_sample_by_id.side_effect = EntityNotFoundError(
            "Sample not found"
        )

        # Act
        result = await sample_service.get_sample(
            sample_id=999, fermentation_id=1, winery_id=100
        )
    ...
    @pytest.mark.asyncio
    async def test_get_sample_repository_error(self, sample_service, mock_sample_repo):
        """Should propagate RepositoryError from repository."""
        # Arrange
        from src.modules.fermentation.src.repository_component.errors import (
            RepositoryError,
        ...
        mock_sample_repo.get_sample_by_id.side_effect = RepositoryError(
            "Database error"
        )
...
class TestGetSamplesByFermentation:
    ...
    async def test_get_samples_by_fermentation_not_found(
        self, sample_service, mock_fermentation_repo
    ):
        """Should raise NotFoundError when fermentation doesn't exist."""
        # Arrange
        mock_fermentation_repo.get_by_id.return_value = None
        ...
        with pytest.raises(NotFoundError, match="Fermentation .* not found"):
            await sample_service.get_samples_by_fermentation(
                fermentation_id=999, winery_id=100
            )
    ...
    async def test_get_samples_by_fermentation_repository_error(
        ...
        mock_fermentation_repo.get_by_id.return_value = sample_fermentation
        mock_sample_repo.get_samples_by_fermentation_id.side_effect = RepositoryError(
            "Database error"
        )
...
class TestGetLatestSample:
    ...
    async def test_get_latest_sample_happy_path(
        ...
        assert result == sample_entity
        mock_fermentation_repo.get_by_id.assert_awaited_once()
        mock_sample_repo.get_latest_sample.assert_awaited_once_with(fermentation_id=1)
    ...
    async def test_get_latest_sample_with_type_filter(
        ...
        # Act
        result = await sample_service.get_latest_sample(
            fermentation_id=1, winery_id=100, sample_type=SampleType.SUGAR
        )

        # Assert
        assert result == sample_entity
        mock_sample_repo.get_latest_sample_by_type.assert_awaited_once_with(
            fermentation_id=1, sample_type=SampleType.SUGAR
        )
    ...
    async def test_get_latest_sample_fermentation_not_found(
        ...
        # Act & Assert
        with pytest.raises(NotFoundError, match="Fermentation .* not found"):
            await sample_service.get_latest_sample(fermentation_id=999, winery_id=100)
    ...
    async def test_get_latest_sample_repository_error(
        ...
        mock_fermentation_repo.get_by_id.return_value = sample_fermentation
        mock_sample_repo.get_latest_sample.side_effect = RepositoryError(
            "Database error"
        )

        # Act & Assert
        with pytest.raises(RepositoryError, match="Database error"):
            await sample_service.get_latest_sample(fermentation_id=1, winery_id=100)
...
class TestGetSamplesInTimerange:
    ...
    async def test_get_samples_in_timerange_happy_path(
        ...
        result = await sample_service.get_samples_in_timerange(
            fermentation_id=1, winery_id=100, start=start, end=end
        )
        ...
        mock_sample_repo.get_samples_in_timerange.assert_awaited_once_with(
            fermentation_id=1, start_time=start, end_time=end
        )

    @pytest.mark.asyncio
    async def test_get_samples_in_timerange_invalid_range(self, sample_service):
    ...
    async def test_get_samples_in_timerange_fermentation_not_found(
        ...
        with pytest.raises(NotFoundError, match="Fermentation .* not found"):
            await sample_service.get_samples_in_timerange(
                fermentation_id=999, winery_id=100, start=start, end=end
            )
    ...
    async def test_get_samples_in_timerange_repository_error(
        ...
        mock_fermentation_repo.get_by_id.return_value = sample_fermentation
        mock_sample_repo.get_samples_in_timerange.side_effect = RepositoryError(
            "Database error"
        )
...
class TestValidateSampleData:
    ...
    async def test_validate_sample_data_valid(
        ...
        result = await sample_service.validate_sample_data(
            fermentation_id=1, winery_id=1, data=sample_create_dto
        )
    ...
    async def test_validate_sample_data_fermentation_not_found(
        ...
        with pytest.raises(NotFoundError, match="Fermentation .* not found"):
            await sample_service.validate_sample_data(
                fermentation_id=999, winery_id=1, data=sample_create_dto
            )
