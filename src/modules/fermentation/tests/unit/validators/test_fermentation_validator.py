...
class TestValidateCompletionCriteria:
    ...
    def test_short_duration_fails(self, validator: FermentationValidator):
        ...
        # Act
        result = validator.validate_completion_criteria(
            input_mass_kg=1000.0, final_sugar_brix=2.5, duration_days=3  # ❌ Too short
        )

        # Assert
        assert result.is_valid is False
        assert any(
            error.field == "duration" and "7 days" in error.message
    ...
    def test_high_residual_sugar_fails(self, validator: FermentationValidator):
        ...
        # Act
        result = validator.validate_completion_criteria(
            input_mass_kg=1000.0, final_sugar_brix=8.0, duration_days=14  # ❌ Too high
        )

        # Assert
        assert result.is_valid is False
        assert any(
            error.field == "final_sugar_brix" and "too high" in error.message.lower()
            for error in result.errors
        )
