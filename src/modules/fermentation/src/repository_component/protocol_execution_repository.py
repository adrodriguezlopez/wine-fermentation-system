...
class ProtocolExecutionRepository(IProtocolExecutionRepository):
    ...
    async def get_by_status(
        self, status: ProtocolExecutionStatus
        ...
        stmt = (
            ...
            .where(ProtocolExecution.status == status)
            ...
            .where(
                ...
                & (
                    ProtocolExecution.status.in_(
                        [ProtocolExecutionStatus.ACTIVE, ProtocolExecutionStatus.PAUSED]
                    )
                )
            )
            .order_by(ProtocolExecution.id)
    ...
    async def list_by_winery_paginated(
        ...
        stmt = (
            select(ProtocolExecution)
            .where(ProtocolExecution.winery_id == winery_id)
            .order_by(ProtocolExecution.id.desc())
            .offset(offset)
            .limit(page_size)
        )
