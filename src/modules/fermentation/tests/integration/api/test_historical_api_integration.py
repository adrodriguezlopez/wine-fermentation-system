...
class TestListHistoricalFermentations:
    ...
    async def test_list_filters_by_date_range(
        ...
        response = await test_client.get(
            ...
            params={"start_date_from": "2023-09-02", "start_date_to": "2023-09-04"},
        )
    ...
    async def test_list_filters_by_status(
        ...
        response = await test_client.get(
            "/api/v1/fermentation/historical", params={"status": "COMPLETED"}
        )
    ...
    async def test_list_pagination_works(
        ...
        response1 = await test_client.get(
            "/api/v1/fermentation/historical", params={"limit": 2, "offset": 0}
        )
        ...
        response2 = await test_client.get(
            "/api/v1/fermentation/historical", params={"limit": 2, "offset": 2}
        )
...
class TestGetFermentationSamples:
    ...
    async def test_get_samples_returns_historical_samples_only(
        ...
        assert (
            len(data) == 4
        )  # Should only return HISTORICAL samples (4), not the manual one
        for sample in data:
...
class TestExtractPatterns:
    ...
    async def test_extract_patterns_aggregates_real_data(
        self,
        test_client,
        multiple_historical_fermentations,
        test_models_with_samples,
        db_session,
        test_user,
    ):
    ...
    async def test_extract_patterns_filters_by_date_range(
        ...
        response = await test_client.get(
            "/api/v1/fermentation/historical/patterns/extract",
            params={"start_date": "2023-09-01", "end_date": "2023-09-03"},
        )
