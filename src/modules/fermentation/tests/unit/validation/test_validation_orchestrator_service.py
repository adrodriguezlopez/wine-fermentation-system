...
from service_component.interfaces.chronology_validation_service_interface import (
    IChronologyValidationService,
)
from service_component.models.schemas.validations.validation_error import (
    ValidationError,
)
from service_component.models.schemas.validations.validation_result import (
    ValidationResult,
)

sys.path.insert(0, str(Path(__file__).parent.parent.parent.parent / "src"))

from service_component.interfaces.business_rule_validation_service_interface import (
    IBusinessRuleValidationService,
)
from service_component.interfaces.value_validation_service_interface import (
...
async def test_validate_sample_complete_chronology_failure(
    ...
    mock_chronology_service.validate_sample_chronology = AsyncMock(
        return_value=ValidationResult.failure(
            errors=[
                ValidationError(
                    field="recorded_at",
                    message="Timestamp is before last sample",
                    current_value="2023-10-01T10:00:00Z",
                )
            ]
        )
    )
    ...
    mock_value_validation_service.validate_sample_value = Mock(
        return_value=ValidationResult.success()
    )
    mock_business_rule_validation_service.validate_sugar_trend = AsyncMock(
        return_value=ValidationResult.success()
    )
    mock_business_rule_validation_service.validate_temperature_range = AsyncMock(
        return_value=ValidationResult.success()
    )

    result = await validation_orchestrator.validate_sample_complete(
        fermentation_id=1, new_sample=Mock()
    )
...
async def test_validate_sample_complete_chronology_and_validation_value_complete_for_sugar(
    ...
    result = await validation_orchestrator.validate_sample_complete(
        fermentation_id=1, new_sample=sample
    )
