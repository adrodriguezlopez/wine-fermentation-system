...
class HistoricalFermentationQueryRequest(BaseModel):
    ...
    start_date_from: Optional[date] = Field(
        None, description="Filter by start date (inclusive)", examples=["2024-01-01"]
    )
    start_date_to: Optional[date] = Field(
        None, description="Filter by start date (inclusive)", examples=["2024-12-31"]
    )
    fruit_origin_id: Optional[int] = Field(
        None, description="Filter by fruit origin ID", examples=[5]
    )
    ...
    limit: int = Field(
        100, ge=1, le=1000, description="Maximum number of results", examples=[100]
    )
    offset: int = Field(0, ge=0, description="Offset for pagination", examples=[0])
