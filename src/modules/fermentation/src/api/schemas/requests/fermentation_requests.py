...
class FermentationCreateRequest(BaseModel):
    ...
    vintage_year: int = Field(
        ..., ge=1900, le=2100, description="Harvest year (1900-2100)"
    )

    yeast_strain: str = Field(
        ..., min_length=1, max_length=100, description="Yeast strain identifier"
    )

    vessel_code: Optional[str] = Field(
        None, max_length=50, description="Optional vessel/tank identifier"
    )

    input_mass_kg: float = Field(
        ..., gt=0, description="Initial grape mass in kilograms (must be positive)"
    )

    initial_sugar_brix: float = Field(
        ..., ge=0, le=50, description="Initial sugar content in °Brix (0-50)"
    )

    initial_density: float = Field(
        ..., gt=0, description="Initial must density (must be positive)"
    )
...
class FermentationUpdateRequest(BaseModel):
    ...
    status: Optional[str] = Field(
        None, max_length=20, description="Fermentation status"
    )

    yeast_strain: Optional[str] = Field(
        None, min_length=1, max_length=100, description="Yeast strain identifier"
    )

    vessel_code: Optional[str] = Field(
        None, max_length=50, description="Vessel/tank identifier"
    )

    input_mass_kg: Optional[float] = Field(
        None, gt=0, description="Initial grape mass in kilograms"
    )

    initial_sugar_brix: Optional[float] = Field(
        None, ge=0, le=50, description="Initial sugar content in °Brix"
    )

    initial_density: Optional[float] = Field(
        None, gt=0, description="Initial must density"
    )

    vintage_year: Optional[int] = Field(
        None, ge=1900, le=2100, description="Harvest year"
    )

    start_date: Optional[datetime] = Field(
        None, description="Fermentation start date and time"
    )
...
class CompleteFermentationRequest(BaseModel):
    ...
    final_sugar_brix: float = Field(
        ..., ge=0, le=50, description="Final residual sugar in °Brix (0-50)"
    )
    ...
    notes: Optional[str] = Field(
        None, max_length=500, description="Optional completion notes or observations"
    )
...
class LotSourceRequest(BaseModel):
    ...
    harvest_lot_id: int = Field(..., gt=0, description="ID of the harvest lot to use")
    ...
    notes: Optional[str] = Field(
        None, max_length=200, description="Optional notes about this lot's contribution"
    )
...
class FermentationWithBlendCreateRequest(BaseModel):
    ...
    def validate_unique_lots(cls, v: List[LotSourceRequest]) -> List[LotSourceRequest]:
        ...
        if len(lot_ids) != len(set(lot_ids)):
            raise ValueError(
                "Duplicate harvest_lot_id found. Each lot can only be used once."
            )
        return v
