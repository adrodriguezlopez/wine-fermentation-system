...
def mock_user_context():
    ...
    return UserContext(
        user_id=1, email="winemaker@test.com", winery_id=100, role=UserRole.WINEMAKER
    )
...
async def test_create_sample_success(
    mock_user_context, mock_sample_service, sample_entity, sample_create_request
):
...
@pytest.mark.asyncio
async def test_create_sample_converts_sample_type_to_enum(
    mock_user_context, mock_sample_service, sample_entity, sample_create_request
):
...
@pytest.mark.asyncio
async def test_create_sample_not_found_error(
    mock_user_context, mock_sample_service, sample_create_request
):
...
@pytest.mark.asyncio
async def test_create_sample_validation_error(
    mock_user_context, mock_sample_service, sample_create_request
):
...
async def test_list_samples_success(
    mock_user_context, mock_sample_service, sample_entity
):
...
@pytest.mark.asyncio
async def test_list_samples_multi_tenant_isolation(
    mock_user_context, mock_sample_service
):
...
@pytest.mark.asyncio
async def test_get_latest_sample_success(
    mock_user_context, mock_sample_service, sample_entity
):
    ...
    mock_sample_service.get_latest_sample.assert_called_once_with(
        fermentation_id=10, winery_id=100, sample_type=None
    )
...
async def test_get_latest_sample_not_found(mock_user_context, mock_sample_service):
    ...
    with pytest.raises(
        Exception
    ) as exc_info:  # Using Exception instead of SampleNotFound
        await get_latest_sample(
            fermentation_id=10,
            sample_type=None,
            current_user=mock_user_context,
...
async def test_get_sample_success(
    ...
    mock_sample_service.get_sample.assert_called_once_with(
        sample_id=1, fermentation_id=10, winery_id=100
    )
...
@pytest.mark.asyncio
async def test_create_sample_with_all_sample_types(
    mock_user_context, mock_sample_service, sample_entity
):
...
@pytest.mark.asyncio
async def test_get_latest_sample_case_insensitive_type(
    mock_user_context, mock_sample_service, sample_entity
):
...
@pytest.mark.asyncio
async def test_get_samples_by_timerange_success(
    mock_user_context, mock_sample_service, sample_entity
):
    """Test getting samples in time range"""
    from src.modules.fermentation.src.api.routers.sample_router import (
        get_samples_by_timerange,
    )
    ...
    mock_sample_service.get_samples_in_timerange.assert_called_once_with(
        fermentation_id=10, winery_id=100, start=start, end=end
    )
...
@pytest.mark.asyncio
async def test_validate_sample_success(
    mock_user_context, mock_sample_service, sample_create_request
):
...
@pytest.mark.asyncio
async def test_validate_sample_with_errors(
    mock_user_context, mock_sample_service, sample_create_request
):
