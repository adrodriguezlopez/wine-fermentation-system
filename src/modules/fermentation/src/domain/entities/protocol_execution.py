...
class ProtocolExecution(BaseEntity):
    ...
    __table_args__ = (
        UniqueConstraint(
            "fermentation_id", name="uq_protocol_executions__fermentation"
        ),
    ...
    fermentation_id: Mapped[int] = mapped_column(
        ForeignKey("fermentations.id"), nullable=False, index=True, unique=True
    )
    ...
    )  # NOT_STARTED, ACTIVE, COMPLETED, etc.
    ...
    skipped_critical_steps: Mapped[int] = mapped_column(
        Integer, default=0, nullable=False
    )  # Count
