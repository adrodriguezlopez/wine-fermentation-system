...
class ProtocolCreateRequest(BaseModel):
    ...
    varietal_code: str = Field(
        ..., min_length=1, max_length=4, description="Varietal code (e.g., 'PN', 'CS')"
    )
    ...
    version: str = Field(..., description="Semantic version (e.g., '1.0', '2.1')")

    protocol_name: str = Field(
        ..., min_length=1, max_length=255, description="Human-readable protocol name"
    )

    expected_duration_days: int = Field(
        ..., gt=0, description="Expected fermentation duration in days"
    )

    description: Optional[str] = Field(
        None, max_length=2000, description="Detailed protocol description"
    )
...
class ProtocolUpdateRequest(BaseModel):
    ...
    protocol_name: Optional[str] = Field(
        None, min_length=1, max_length=255, description="Updated protocol name"
    )
    ...
    expected_duration_days: Optional[int] = Field(
        None, gt=0, description="Updated expected duration"
    )
...
class StepCreateRequest(BaseModel):
    ...
    step_order: int = Field(..., gt=0, description="Position in protocol (1-indexed)")
    ...
    description: str = Field(
        ..., min_length=1, max_length=500, description="Specific step details"
    )

    expected_day: int = Field(..., ge=0, description="Day when step occurs")

    tolerance_hours: int = Field(
        ..., ge=0, description="Hours before/after expected day"
    )

    duration_minutes: int = Field(..., gt=0, description="How long step takes")

    criticality_score: float = Field(
        ..., ge=0, le=100, description="Importance score 0-100"
    )

    can_repeat_daily: bool = Field(False, description="Whether step repeats each day")

    depends_on_step_id: Optional[int] = Field(
        None, description="Optional dependency on previous step"
    )
...
class StepUpdateRequest(BaseModel):
    ...
    description: Optional[str] = Field(
        None, min_length=1, max_length=500, description="Updated step details"
    )

    expected_day: Optional[int] = Field(None, ge=0, description="Updated day")

    tolerance_hours: Optional[int] = Field(None, ge=0, description="Updated tolerance")

    duration_minutes: Optional[int] = Field(None, gt=0, description="Updated duration")

    criticality_score: Optional[float] = Field(
        None, ge=0, le=100, description="Updated criticality"
    )
...
class ExecutionStartRequest(BaseModel):
    ...
    protocol_id: int = Field(..., gt=0, description="Protocol to execute")

    start_date: Optional[datetime] = Field(None, description="When execution starts")
...
class ExecutionUpdateRequest(BaseModel):
    ...
    compliance_score: Optional[float] = Field(
        None, ge=0, le=100, description="Updated compliance score"
    )

    notes: Optional[str] = Field(None, max_length=1000, description="Execution notes")


class CompletionCreateRequest(BaseModel):
    """Request DTO for marking a step complete or skipped"""

    step_id: int = Field(..., gt=0, description="Step being completed")

    was_skipped: bool = Field(False, description="Whether step was skipped")

    completed_at: Optional[datetime] = Field(
        None, description="When step was completed (required if not skipped)"
    )

    is_on_schedule: bool = Field(True, description="Whether completed on expected day")

    days_late: int = Field(0, ge=0, description="Days past expected date")

    skip_reason: Optional[str] = Field(
        None, description="Why step was skipped (required if was_skipped=True)"
    )

    skip_notes: Optional[str] = Field(
        None, max_length=500, description="Notes about skip"
    )
