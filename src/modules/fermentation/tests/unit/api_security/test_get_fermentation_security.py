...
def user_context_winery_100():
    ...
    return UserContext(user_id=1, email="test@test.com", winery_id=100, role="user")


@pytest.fixture
def user_context_winery_200():
    """User from winery 200 (different winery)"""
    return UserContext(user_id=2, email="other@test.com", winery_id=200, role="user")
...
class TestGetFermentationSecurity:
    ...
    async def test_get_fermentation_same_winery_success(
        self, user_context_winery_100, fermentation_winery_100
    ):
        """
        ...
        response = await get_fermentation(
            fermentation_id=1, current_user=user_context_winery_100, service=service
        )
    ...
    async def test_get_fermentation_cross_winery_defense_in_depth(
        self, user_context_winery_100, fermentation_winery_200
    ):
        """
