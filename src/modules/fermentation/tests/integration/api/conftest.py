...
async def fermentation_service(db_session):
    ...
    return FermentationService(fermentation_repo=fermentation_repo, validator=validator)
...
async def test_client(
    app, fermentation_service, pattern_analysis_service, sample_service, test_user
):
...
def test_models_with_samples(test_models):
    ...
    """
    from src.modules.fermentation.src.domain.entities.samples.sugar_sample import (
        SugarSample,
    )
    from src.modules.fermentation.src.domain.entities.samples.density_sample import (
        DensitySample,
    )
    from src.modules.fermentation.src.domain.entities.samples.celcius_temperature_sample import (
...
async def historical_samples(
    test_models_with_samples, db_session, historical_fermentation, test_user
):
...
async def other_winery_fermentation(test_models, db_session):
    ...
    other_winery = Winery(
        code=f"OTHER-{uuid4().hex[:8].upper()}", name="Other Winery", location="Sonoma"
    )
