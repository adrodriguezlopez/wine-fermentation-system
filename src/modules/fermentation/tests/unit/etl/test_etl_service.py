...
from src.modules.fermentation.src.service_component.etl.etl_service import (
    ETLService,
    ImportResult,
)
from src.modules.fermentation.src.domain.enums.data_source import DataSource
from src.modules.fermentation.src.domain.interfaces.harvest_lot_provider_interface import (
    IHarvestLotProvider,
)
...
class TestETLService:
    ...
    def patch_entities(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "src.modules.fermentation.src.service_component.etl.etl_service.DensitySample",
            mock_density,
        )
        monkeypatch.setattr(
            "src.modules.fermentation.src.service_component.etl.etl_service.CelsiusTemperatureSample",
            mock_temp,
        )
        monkeypatch.setattr(
            "src.modules.fermentation.src.service_component.etl.etl_service.SugarSample",
            mock_sugar,
        )
    ...
    def etl_service(
        ...
        service._patchers = [
            patch(
                "src.modules.fermentation.src.service_component.etl.etl_service.FermentationRepository",
                return_value=mock_fermentation_repo,
            ),
            patch(
                "src.modules.fermentation.src.service_component.etl.etl_service.SampleRepository",
                return_value=mock_sample_repo,
            ),
            patch(
                "src.modules.fermentation.src.service_component.etl.etl_service.LotSourceRepository",
    ...
    @pytest.mark.asyncio
    async def test_imports_valid_single_fermentation_with_samples(
        self, etl_service, tmp_path
    ):
        ...
        assert (
            result.samples_created == 4
        )  # 2 rows * 2 samples each (density, temperature)
    ...
    @pytest.mark.asyncio
    async def test_calls_repository_create_methods(
        self, etl_service, mock_fruit_origin_service, tmp_path
    ):
        ...
        assert (
            etl_service._mock_sample_repo.create.call_count == 2
        )  # density + temperature
    ...
    async def test_handles_optional_sugar_brix(self, etl_service, tmp_path):
        ...
        assert result.success
        assert (
            result.samples_created == 6
        )  # 2 rows * 3 samples each (density, temp, sugar)
        # Verify 6 sample create calls were made
        assert etl_service._mock_sample_repo.create.call_count == 6

    @pytest.mark.asyncio
    async def test_handles_duplicate_vessel_codes_via_database_constraint(
        self, etl_service, tmp_path
    ):
        ...
        etl_service._mock_fermentation_repo.create.side_effect = Exception(
    ...
    @pytest.mark.asyncio
    async def test_handles_missing_vineyard_name_with_default(
        self, etl_service, mock_fruit_origin_service, tmp_path
    ):
    ...
    @pytest.mark.asyncio
    async def test_handles_empty_vineyard_name_with_default(
        self, etl_service, mock_fruit_origin_service, tmp_path
    ):
    ...
    @pytest.mark.asyncio
    async def test_handles_missing_grape_variety_with_default(
        self, etl_service, mock_fruit_origin_service, tmp_path
    ):
    ...
    @pytest.mark.asyncio
    async def test_handles_both_vineyard_and_variety_missing(
        self, etl_service, mock_fruit_origin_service, tmp_path
    ):
    ...
    @pytest.mark.asyncio
    async def test_uses_provided_winery_and_user_id(
        self, etl_service, mock_fruit_origin_service, tmp_path
    ):
    ...
    @pytest.mark.asyncio
    async def test_partial_success_one_fermentation_fails(
        self,
        mock_session_manager,
        mock_fruit_origin_service,
        mock_fermentation_repo,
        ...
        mock_fruit_origin_service.ensure_harvest_lot_for_import = AsyncMock(
            side_effect=ensure_harvest_lot_side_effect
        )
        ...
        with (
            ...
            assert (
                "Harvest lot creation failed" in result.failed_fermentations[0]["error"]
            )

    @pytest.mark.asyncio
    async def test_partial_success_all_fermentations_fail(
        self,
        mock_session_manager,
        mock_fruit_origin_service,
        mock_fermentation_repo,
    ...
    @pytest.mark.asyncio
    async def test_partial_success_fermentation_create_fails(
        self, etl_service, tmp_path
    ):
        ...
        etl_service._mock_fermentation_repo.create = AsyncMock(
            side_effect=create_fermentation_side_effect
        )
    ...
    @pytest.mark.asyncio
    async def test_progress_callback_invoked_per_fermentation(
        self, etl_service, tmp_path
    ):
        ...
        result = await etl_service.import_file(
            excel_file, winery_id=1, user_id=1, progress_callback=progress_callback
        )
    ...
    @pytest.mark.asyncio
    async def test_cancellation_without_callback_still_works(
        self, etl_service, tmp_path
    ):
