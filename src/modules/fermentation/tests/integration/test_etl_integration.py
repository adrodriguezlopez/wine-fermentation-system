...
class TestETLIntegration:
    ...
    async def test_complete_import_flow_creates_all_entities(
        self, etl_service, sample_excel_file, db_session, test_winery, test_user
    ):
        """
        ...
        result = await etl_service.import_file(
            sample_excel_file, winery_id=test_winery.id, user_id=test_user.id
        )
        ...
        assert (
            result.samples_created == 10
        )  # 5 rows * 2 samples (density + temperature) = 10
    ...
    async def test_vineyard_reuse_across_fermentations(
        ...
        # Import file
        result = await etl_service.import_file(
            excel_file, winery_id=test_winery.id, user_id=test_user.id
        )
        ...
        assert (
            len(lot_list) == 1
        )  # All 5 fermentations share same vineyard + harvest date
    ...
    async def test_shared_default_block_prevents_duplicates(
        ...
        assert (
            len(lot_list) == 1
        )  # All 3 fermentations share same vineyard + harvest date
    ...
    async def test_progress_tracking_with_real_import(
        self, etl_service, sample_excel_file, test_winery, test_user
    ):
        """
    ...
    async def test_cancellation_with_partial_commit(
        ...
        assert (
            len(lots.scalars().all()) == 1
        )  # Both committed fermentations share same vineyard + harvest date
    ...
    async def test_handles_missing_optional_fields(
        ...
        assert all(
            lot.grape_variety == "UNKNOWN" for lot in lot_list
        )  # Missing grape_variety defaults to 'UNKNOWN'
