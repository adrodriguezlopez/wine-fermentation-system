...
async def test_create_fermentation_success(
    mock_user_context, mock_fermentation_service, sample_fermentation_entity
):
    """Should create fermentation successfully."""
    mock_fermentation_service.create_fermentation.return_value = (
    ...
    response = await create_fermentation(
        request, mock_user_context, mock_fermentation_service
    )
...
async def test_create_fermentation_not_found_error(
    mock_user_context, mock_fermentation_service
):
    """Should raise FermentationNotFound (DomainError) for invalid references."""
    mock_fermentation_service.create_fermentation.side_effect = NotFoundError(
        "Vessel not found"
    )
...
async def test_get_fermentation_success(
    ...
    mock_fermentation_service.get_fermentation.assert_called_once_with(
        fermentation_id=1, winery_id=10
    )


@pytest.mark.asyncio
async def test_get_fermentation_not_found(mock_user_context, mock_fermentation_service):
...
async def test_create_fermentation_with_blend_success(
    ...
    mock_fermentation_service.create_fermentation_with_blend.return_value = (
        sample_fermentation_entity
    )
    ...
    response = await create_fermentation_with_blend(
        request, mock_user_context, mock_fermentation_service, mock_unit_of_work
    )
...
async def test_create_fermentation_with_blend_not_found_error(
    mock_user_context, mock_fermentation_service, mock_unit_of_work
):
    """Should raise FermentationNotFound for invalid harvest lot."""
    mock_fermentation_service.create_fermentation_with_blend.side_effect = (
        NotFoundError("Harvest lot not found")
    )
    ...
    request = FermentationWithBlendCreateRequest(
        ...
        initial_density=1.092,
        start_date=date(2024, 3, 15),
        lot_sources=[LotSourceRequest(harvest_lot_id=999, mass_used_kg=100.0)],
    )
...
async def test_list_fermentations_success(
    ...
    response = await list_fermentations(
        mock_user_context, mock_fermentation_service, page=1, size=20
    )
...
async def test_list_fermentations_with_filters(
    ...
    mock_fermentation_service.get_fermentations_by_winery.assert_called_once_with(
        winery_id=10, status="ACTIVE", include_completed=True
    )
...
async def test_list_fermentations_pagination(
    ...
    response = await list_fermentations(
        mock_user_context, mock_fermentation_service, page=2, size=2
    )
...
async def test_update_fermentation_success(
    ...
    mock_fermentation_service.update_fermentation.return_value = (
        sample_fermentation_entity
    )

    request = FermentationUpdateRequest(
        yeast_strain="EC-1118 Updated", vessel_code="TANK-02"
    )
...
async def test_update_fermentation_not_found(
    ...
    mock_fermentation_service.update_fermentation.side_effect = NotFoundError(
        "Fermentation not found"
    )
...
async def test_update_fermentation_status_not_found(
    ...
    """Should raise FermentationNotFound when fermentation doesn't exist."""
    mock_fermentation_service.update_status.side_effect = NotFoundError(
        "Fermentation not found"
    )
...
async def test_complete_fermentation_not_found(
    ...
    """Should raise FermentationNotFound when fermentation doesn't exist."""
    mock_fermentation_service.complete_fermentation.side_effect = NotFoundError(
        "Fermentation not found"
    )

    request = CompleteFermentationRequest(final_sugar_brix=0.5, final_mass_kg=950.0)
...
async def test_delete_fermentation_success(
    ...
    mock_fermentation_service.soft_delete.assert_called_once_with(
        fermentation_id=1, winery_id=10, user_id=1
    )
...
async def test_validate_fermentation_success(
    ...
    mock_fermentation_service.validate_creation_data.return_value = (
...
async def test_get_fermentation_timeline_not_found(
    mock_user_context, mock_fermentation_service, mock_sample_service
):
    """Should raise FermentationNotFound when fermentation doesn't exist."""
    mock_fermentation_service.get_fermentation.return_value = None
