...
# ---------------------------------------------------------------------------
# Protocol Advisory response (ADR-037)
# ---------------------------------------------------------------------------

class ProtocolAdvisoryResponse(BaseModel):
    """
    Response DTO for a Protocol Advisory generated by the Analysis Engine.

    Advisories are produced when anomalies map to protocol step violations,
    giving the winemaker actionable guidance to adjust the active protocol.

    Maps to the ProtocolAdvisory ORM entity (domain/entities/protocol_advisory.py).
    """

    model_config = ConfigDict(from_attributes=True)

    id: UUID = Field(..., description="Advisory unique identifier")
    fermentation_id: UUID = Field(..., description="Fermentation this advisory belongs to")
    analysis_id: UUID = Field(..., description="Analysis run that generated this advisory")
    execution_id: Optional[UUID] = Field(None, description="Protocol execution UUID, if available")

    # Classification
    advisory_type: str = Field(
        ...,
        description="Advisory type: ACCELERATE_STEP | SKIP_STEP | ADD_STEP"
    )
    target_step_type: str = Field(
        ...,
        description="The protocol step type this advisory targets"
    )
    ...
    # Content
    suggestion: str = Field(..., description="Actionable suggestion text for the winemaker")
    reasoning: str = Field(..., description="Why this advisory was generated")
    confidence: float = Field(..., description="Confidence score [0-1]", ge=0, le=1)

    # Acknowledgement
    is_acknowledged: bool = Field(
        ...,
        description="Whether the winemaker has acknowledged this advisory"
    )
    acknowledged_at: Optional[datetime] = Field(
        None,
        description="When acknowledged (UTC), null if not yet acknowledged"
    )
    created_at: datetime = Field(..., description="When the advisory was created (UTC)")


class ProtocolAdvisoryListResponse(BaseModel):
    """Paginated list of protocol advisories."""

    items: List[ProtocolAdvisoryResponse] = Field(..., description="Advisory items")
    total: int = Field(..., description="Total number of advisories", ge=0)
    unacknowledged_count: int = Field(
        ...,
        description="Number of pending (unacknowledged) advisories",
        ge=0
    )
