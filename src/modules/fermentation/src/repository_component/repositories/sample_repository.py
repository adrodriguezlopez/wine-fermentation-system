...
class SampleRepository(BaseRepository, ISampleRepository):
        ...
        async def _create_operation():
            with LogTimer(logger, "create_sample"):
                ...
                async with session_cm as session:
                    if (
                        sample_type_value == SampleType.SUGAR.value
                        or sample.sample_type == SampleType.SUGAR
                    ):
                        ...
                        or sample.sample_type == SampleType.DENSITY
                    ):
                        ...
                        or sample.sample_type == SampleType.TEMPERATURE
                    ):
    ...
    async def get_sample_by_id(
        ...
        if not winery_id or winery_id <= 0:
            raise ValueError(
                f"winery_id is REQUIRED for multi-tenancy security, got: {winery_id}"
            )
        ...
        async def _get_operation():
            with LogTimer(logger, "get_sample_by_id"):
                ...
                from src.modules.fermentation.src.domain.entities.fermentation import (
                    Fermentation,
                )
                ...
                async with session_cm as session:
                    ...
                    for sample_class in [
                        ...
                        stmt = (
                            ...
                            .join(
                                Fermentation,
                                sample_class.fermentation_id == Fermentation.id,
                            )
                            .where(
                                sample_class.id == sample_id,
                                sample_class.fermentation_id == fermentation_id,
                                Fermentation.winery_id
                                == winery_id,  # ADR-025: winery_id validation
                            )
                        )
    ...
    async def get_samples_by_fermentation_id(
        ...
        with LogTimer(logger, "get_samples_by_fermentation"):
            logger.debug(
                "querying_samples_by_fermentation", fermentation_id=fermentation_id
            )
            from src.modules.fermentation.src.domain.entities.samples.sugar_sample import (
                SugarSample,
            )
            ...
            async with session_cm as session:
                ...
                for sample_class in [
                    ...
                    stmt = (
                        select(sample_class)
                        .where(sample_class.fermentation_id == fermentation_id)
    ...
    async def get_fermentation_start_date(self, fermentation_id: int) -> datetime:
        ...
        session_cm = await self.get_session()
        async with session_cm as session:
            stmt = select(Fermentation.start_date).where(
    ...
    async def get_latest_sample_by_type(
        ...
        async with session_cm as session:
            ...
            stmt = (
                select(sample_class)
                .where(sample_class.fermentation_id == fermentation_id)
                .order_by(sample_class.recorded_at.desc())
                .limit(1)
            )
    ...
    async def soft_delete_sample(self, sample_id: int) -> None:
        ...
        async with session_cm as session:
            ...
            for sample_class in [SugarSample, DensitySample, CelsiusTemperatureSample]:
                stmt = (
                    update(sample_class)
                    .where(sample_class.id == sample_id)
