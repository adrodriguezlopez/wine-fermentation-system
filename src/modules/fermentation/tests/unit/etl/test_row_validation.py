...
class TestRowValidation:
    ...
    async def test_validates_required_fermentation_fields(self, tmp_path):
        ...
        assert any(
            "fermentation_start_date" in err.lower() for err in result.row_errors[1]
        )
    ...
    async def test_validates_date_formats(self, tmp_path):
        ...
        assert any(
            "fermentation_start_date" in err.lower() for err in result.row_errors[0]
        )
        assert any(
            "fermentation_end_date" in err.lower() for err in result.row_errors[1]
        )
    ...
    async def test_validates_density_range(self, tmp_path):
        ...
        assert result.invalid_row_count == 2
        assert any(
            "density" in err.lower() and "range" in err.lower()
            for err in result.row_errors[0]
        )
        assert any(
            "density" in err.lower() and "range" in err.lower()
            for err in result.row_errors[2]
        )
    ...
    async def test_validates_temperature_range(self, tmp_path):
        ...
        assert result.invalid_row_count == 2
        assert any(
            "temperature" in err.lower() and "range" in err.lower()
            for err in result.row_errors[0]
        )
        assert any(
            "temperature" in err.lower() and "range" in err.lower()
    ...
    async def test_validates_sugar_brix_range_when_present(self, tmp_path):
        ...
        assert result.invalid_row_count == 2
        assert any(
            "sugar" in err.lower() or "brix" in err.lower()
            for err in result.row_errors[0]
        )
        assert any(
            "sugar" in err.lower() or "brix" in err.lower()
    ...
    async def test_validates_harvest_mass_positive(self, tmp_path):
        ...
        assert result.invalid_row_count == 2
        assert any(
            "harvest_mass" in err.lower() and "positive" in err.lower()
            for err in result.row_errors[0]
        )
        assert any(
            "harvest_mass" in err.lower() and "positive" in err.lower()
            for err in result.row_errors[1]
        )
    ...
    async def test_validates_numeric_types(self, tmp_path):
        ...
        assert result.invalid_row_count == 2
        assert any(
            "harvest_mass" in err.lower() and "numeric" in err.lower()
            for err in result.row_errors[0]
        )
        assert any(
            "density" in err.lower() and "numeric" in err.lower()
            for err in result.row_errors[1]
        )
