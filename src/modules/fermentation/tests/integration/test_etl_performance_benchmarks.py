...
def large_excel_file_same_vineyard(tmp_path):
    ...
    for i in range(100):
        ...
        harvest_date = base_date + timedelta(
            days=i
        )  # Unique date per fermentation (100 harvest lots)
...
class TestETLPerformanceBenchmarks:
    ...
    async def test_batch_vineyard_loading_eliminates_n_plus_1_queries(
        ...
        assert (
            result.samples_created == 600
        )  # 100 fermentations × 2 rows × 3 sample types (density, temp, sugar)
    ...
    async def test_shared_default_block_reduces_database_records(
        ...
        assert (
            len(block_ids) == 1
        ), f"Expected all lots to share 1 block, found {len(block_ids)}"
    ...
    async def test_multiple_vineyards_batch_loading(
        ...
        assert (
            result.samples_created == 600
        )  # 100 fermentations × 2 rows × 3 sample types
        ...
        assert (
            len(blocks) == 10
        ), f"Expected 10 blocks (1 per vineyard), found {len(blocks)}"

        # Assert - Each vineyard has exactly 1 block
        block_vineyard_ids = [block.vineyard_id for block in blocks]
        assert (
            len(set(block_vineyard_ids)) == 10
        ), "Each vineyard should have exactly 1 block"
    ...
    async def test_large_import_with_progress_tracking(
        ...
        assert (
            duration < 35
        ), f"Import with progress took {duration:.2f}s, expected < 35s"
...
class TestETLPerformanceComparison:
    ...
    async def test_performance_comparison_batch_vs_individual_vineyard_queries(
        self, etl_service, test_winery, db_session
    ):
        """
        ...
        vineyard_cache = await etl_service.fruit_origin_service.batch_load_vineyards(
            codes=vineyard_names, winery_id=test_winery.id
        )
        batch_duration = time.time() - start_batch

        # Assert batch loading succeeded
        assert (
            len(vineyard_cache) == 20
        ), f"Expected 20 vineyards, got {len(vineyard_cache)}"
