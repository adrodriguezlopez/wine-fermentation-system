                ...
                rows.append(
                    {
                        ...
                        "fermentation_start_date": fermentation_start.strftime(
                            "%Y-%m-%d"
                        ),
            ...
            rows.append(
                {
                    ...
                    "temperature_celsius": (
                        temperature_value if sample_idx % 2 == 1 else None
                    ),
                }
            )
...
def etl_service(db_session):
    ...
    return ETLService(
        session_manager=session_manager, fruit_origin_service=fruit_origin_service
    )
...
class TestETLLoadTesting:
    ...
    async def test_load_1000_fermentations_multiple_vineyards(
        self, etl_service, db_session, large_dataset_multiple_vineyards
    ):
        """
        ...
        # Validate results
        assert (
            result.fermentations_created == 1000
        ), "All 1000 fermentations should import successfully"
        assert (
            len(result.failed_fermentations) == 0
        ), "No failures expected with valid data"
        ...
        print(
            f"✓ Imported 1000 fermentations in {elapsed_time:.2f} seconds ({1000/elapsed_time:.1f} fermentations/sec)"
        )
        ...
        stmt = (
            ...
            .where(Fermentation.winery_id == winery_id)
        )
        result_count = await db_session.scalar(stmt)
        assert (
            result_count == 1000
        ), f"Expected 1000 fermentations in DB, found {result_count}"

        # Vineyard optimization check (should create only 10 vineyards)
        stmt = (
            select(func.count())
            .select_from(Vineyard)
            .where(Vineyard.winery_id == winery_id)
        )
        ...
        assert (
            block_count <= 100
        ), f"Expected <= 100 blocks (shared defaults), found {block_count}"

        print(
            f"✓ Database optimizations working: 10 vineyards, {block_count} blocks (shared across fermentations)"
        )

    async def test_load_partial_success_with_errors(
        self, etl_service, db_session, mixed_valid_invalid_dataset
    ):
        """
        ...
        start_time = time.time()
        result = await etl_service.import_file(
            file_path=mixed_valid_invalid_dataset, winery_id=winery_id, user_id=1
        )
        elapsed_time = time.time() - start_time
        ...
        ), f"Expected row_validation failure, got {result.phase_failed}"
        assert (
            result.fermentations_created == 0
        ), "No fermentations should be created when validation fails"
        ...
        assert (
            elapsed_time < 10.0
        ), f"Validation took {elapsed_time:.2f}s, should be < 10s"

        print(
            f"✓ Validation correctly rejected file with invalid data in {elapsed_time:.2f}s"
        )
        ...
        assert (
            elapsed_time < 90.0
        ), f"Import with errors took {elapsed_time:.2f}s, should be < 90s"
        print(
            f"✓ Processed 1000 records (partial success) in {elapsed_time:.2f} seconds"
        )
        ...
        assert (
            db_count == result.fermentations_created
        ...
        for failed in result.failed_fermentations[:10]:  # Check first 10
            stmt = select(Fermentation).where(
                Fermentation.winery_id == winery_id, Fermentation.code == failed["code"]
            )
            fermentation = await db_session.scalar(stmt)
            assert (
                fermentation is None
    ...
    async def test_load_progress_tracking_accuracy(
        ...
        assert (
            len(progress_events) >= 10
        ), "Should have multiple progress updates for 1000 records"

        # Check total consistency
        for event in progress_events:
            assert (
                event["total"] == 1000
            ), f"Total should always be 1000, got {event['total']}"

        # Check monotonic progress (current should always increase or stay same)
        for i in range(1, len(progress_events)):
            assert (
                progress_events[i]["current"] >= progress_events[i - 1]["current"]
        ...
        print(
            f"  - Event frequency: ~{1000 / len(progress_events):.0f} fermentations per update"
        )
    ...
    async def test_load_cancellation_during_import(
        ...
        assert (
            "cancelled" in str(exc_info).lower()
        ), "Exception should mention cancellation"
        ...
        assert (
            400 <= imported_count <= 600
        ), f"Expected ~500 fermentations imported before cancel, got {imported_count}"

        print(
            f"✓ Cancellation working: {imported_count} fermentations imported before cancel"
        )
    ...
    async def test_load_memory_efficiency(
        ...
        start_time = time.time()
        result = await etl_service.import_file(
            file_path=large_dataset_multiple_vineyards, winery_id=winery_id, user_id=1
        )
        elapsed_time = time.time() - start_time
        ...
        assert (
            elapsed_time < 60.0
        ), f"Slow performance ({elapsed_time:.2f}s) may indicate memory issues"

        print(
            f"✓ Memory efficiency check passed: 1000 fermentations in {elapsed_time:.2f}s"
        )
        ...
        stmt = (
            select(Fermentation.vessel_code, func.count())
            .where(Fermentation.winery_id == winery_id)
            .group_by(Fermentation.vessel_code)
            .having(func.count() > 1)
        )
        result = await db_session.execute(stmt)
        ...
        assert (
            len(duplicates) == 0
        ), f"Found {len(duplicates)} duplicate fermentation codes"
    ...
    async def test_load_concurrent_vineyard_access(
        ...
        stmt = (
            select(Vineyard.code, func.count())
            .join(VineyardBlock, Vineyard.id == VineyardBlock.vineyard_id)
            .join(HarvestLot, VineyardBlock.id == HarvestLot.block_id)
            .where(Vineyard.winery_id == winery_id)
        ...
        assert (
            len(vineyard_distribution) == 10
        ), f"Expected 10 vineyards, found {len(vineyard_distribution)}"
...
class TestETLStressScenarios:
    ...
    async def test_stress_duplicate_fermentation_codes(
        self, etl_service, db_session, tmp_path
    ):
        """
        ...
        # Execute import
        result = await etl_service.import_file(
            file_path=file_path, winery_id=1, user_id=1
        )
        ...
        if result.success == False and result.phase_failed == "row_validation":
            ...
            assert (
                result.fermentations_created == 0
            ), "No records should be created when validation fails"
        else:
            # Some records were created, check database consistency
            # First occurrence succeeds, second fails due to UNIQUE constraint
            total_processed = result.fermentations_created + len(
                result.failed_fermentations
            )
            print(
                f"✓ Duplicate handling at database level: {result.fermentations_created} unique, {len(result.failed_fermentations)} duplicates rejected"
            )
            assert (
                result.fermentations_created > 0
            ), "Should have created some fermentations"
            assert (
                result.fermentations_created <= 500
            ), f"Should have at most 500 unique, got {result.fermentations_created}"
