...
class TestProtocolExecutionEntity:
    ...
    def test_execution_status_valid_values(self):
        ...
        for status in statuses:
            execution = ProtocolExecution(
                fermentation_id=1, protocol_id=1, winery_id=1, status=status
            )
...
class TestSkipReasonEnum:
    ...
    def test_skip_reason_can_be_used_in_completion(self):
        ...
        for reason in SkipReason:
            completion = StepCompletion(
                execution_id=1, step_id=1, was_skipped=True, skip_reason=reason
            )
...
class TestEntityValidation:
    ...
    def test_execution_compliance_score_is_numeric(self):
        ...
        execution = ProtocolExecution(
            fermentation_id=1, protocol_id=1, winery_id=1, compliance_score=87.5
        )
        assert isinstance(execution.compliance_score, float) or isinstance(
            execution.compliance_score, int
        )
