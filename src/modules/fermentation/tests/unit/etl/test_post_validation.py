...
class TestPostValidation:
    ...
    async def test_validates_fermentation_date_chronology(self, tmp_path):
        ...
        df = pd.DataFrame(
            {
                ...
                "fermentation_end_date": [
                    ...
                    "2023-03-15",
                    "2023-04-10",
                ],  # Row 1: end before start
                "harvest_date": [
                    "2023-03-05",
                    "2023-03-05",
                    "2023-03-10",
                ],  # Row 2: harvest after start
        ...
        assert any(
            "date" in err.lower() and "chronology" in err.lower()
            for err in result.row_errors[1]
        )
        assert any(
            "harvest" in err.lower() and "start" in err.lower()
    ...
    async def test_validates_sample_date_after_fermentation_start(self, tmp_path):
        ...
        assert any(
            "sample" in err.lower() and "before" in err.lower()
    ...
    async def test_validates_sample_date_before_fermentation_end(self, tmp_path):
        ...
        assert 0 in result.row_errors
        assert any(
            "sample" in err.lower() and "after" in err.lower()
            for err in result.row_errors[0]
        )

    @pytest.mark.asyncio
    async def test_validates_samples_in_chronological_order_per_fermentation(
        self, tmp_path
    ):
        ...
        df = pd.DataFrame(
            {
                ...
                "sample_date": [
                    ...
                    "2023-03-15",
                    "2023-03-18",
                ],  # FERM-001 row 1 is out of order
        ...
        assert 1 in result.row_errors
        assert any(
            "chronological" in err.lower() or "order" in err.lower()
            for err in result.row_errors[1]
        )
