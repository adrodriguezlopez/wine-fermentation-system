...
class HistoricalDataService:
    ...
    **Removal Date:** Approximately February 1, 2026 (2 weeks after deprecation)
    ...
    async def get_historical_fermentations(
        self, winery_id: int, filters: Dict[str, Any], limit: int = 100, offset: int = 0
    ) -> List[Fermentation]:
        """
        ...
        if "fruit_origin_id" in filters:
            fermentations = [
                f
                for f in fermentations
                if f.fruit_origin_id == filters["fruit_origin_id"]
            ]
    ...
    async def get_historical_fermentation_by_id(
        ...
        if not fermentation:
            logger.warning("fermentation_not_found", fermentation_id=fermentation_id)
