...
class FermentationProtocolRepository(IFermentationProtocolRepository):
    ...
    async def get_by_id(self, protocol_id: int) -> Optional[FermentationProtocol]:
        ...
        stmt = select(FermentationProtocol).where(
            FermentationProtocol.id == protocol_id
        )
    ...
    async def list_by_winery_paginated(
        ...
        stmt = (
            select(FermentationProtocol)
            .where(FermentationProtocol.winery_id == winery_id)
            .order_by(
                FermentationProtocol.varietal_name, FermentationProtocol.version.desc()
            )
            .offset(offset)
            .limit(page_size)
        )
    ...
    async def list_templates_paginated(
        ...
        """
        base_filter = (FermentationProtocol.winery_id == winery_id) & (
