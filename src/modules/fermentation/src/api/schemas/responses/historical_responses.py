...
class HistoricalSampleResponse(BaseModel):
    ...
    sample_type: str = Field(
        ..., description="Type of sample (density, sugar, temperature, etc.)"
    )
    ...
    density: Optional[float] = Field(
        None, description="Density value (for density samples)"
    )
    sugar_brix: Optional[float] = Field(
        None, description="Sugar in °Brix (for sugar samples)"
    )
    temperature_celsius: Optional[float] = Field(
        None, description="Temperature in Celsius (for temperature samples)"
    )
    ...
    def from_entity(cls, entity: BaseSample) -> "HistoricalSampleResponse":
        ...
        data = {
            ...
            "sample_type": (
                entity.sample_type.value
                if hasattr(entity.sample_type, "value")
                else str(entity.sample_type)
            ),
        ...
        if (
            hasattr(entity, "temperature_celsius")
            and entity.temperature_celsius is not None
        ):
...
class HistoricalFermentationResponse(BaseModel):
    ...
    initial_sugar_brix: Optional[float] = Field(
        None, description="Initial sugar in °Brix"
    )
...
class PaginatedHistoricalFermentationsResponse(BaseModel):
    ...
    items: List[HistoricalFermentationResponse] = Field(
        ..., description="List of historical fermentations"
    )
...
class PatternResponse(BaseModel):
    ...
    avg_initial_density: Optional[float] = Field(
        None, description="Average initial density"
    )
    avg_final_density: Optional[float] = Field(
        None, description="Average final density"
    )
    avg_initial_sugar_brix: Optional[float] = Field(
        None, description="Average initial sugar (°Brix)"
    )
    avg_final_sugar_brix: Optional[float] = Field(
        None, description="Average final sugar (°Brix)"
    )
    avg_duration_days: Optional[float] = Field(
        None, description="Average fermentation duration in days"
    )
...
class StatisticsResponse(BaseModel):
    ...
    avg_duration_days: Optional[float] = Field(
        None, description="Average duration in days"
    )
    success_rate: float = Field(..., description="Success rate (0.0-1.0)")
    total_volume_liters: Optional[float] = Field(
        None, description="Total volume processed (liters)"
    )
    most_common_yeast: Optional[str] = Field(
        None, description="Most commonly used yeast strain"
    )
